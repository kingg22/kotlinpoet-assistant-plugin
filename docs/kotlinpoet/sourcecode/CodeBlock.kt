 ...
 *  * `»` ends a statement.
 */
public class CodeBlock private constructor(internal val formatParts: List<String>, internal val args: List<Any?>) {
    ...
    public class Builder {
        ...
        private fun argToName(o: Any?) = when (o) {
            ...
            is ContextParameter -> if (o.name ==
                "_"
            ) {
                throw IllegalStateException("Named context parameter required")
            } else {
                o.name
            }
    ...
    public companion object {
        ...
        @JvmStatic public fun of(format: String, vararg args: Any?): CodeBlock = Builder().add(format, *args).build()
...
): CodeBlock = map(transform).joinToCode(separator, prefix, suffix)
...
public inline fun buildCodeBlock(builderAction: CodeBlock.Builder.() -> Unit): CodeBlock =
    CodeBlock.builder().apply(builderAction).build()
...
public inline fun CodeBlock.Builder.withIndent(builderAction: CodeBlock.Builder.() -> Unit): CodeBlock.Builder =
    indent().also(builderAction).unindent()
